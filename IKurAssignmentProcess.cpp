...
LIBECS_DM_CLASS( IKurAssignmentProcess, Process )
{
 ...
 public:
	...
	virtual void initialize()
	{
		...
		kur    = getVariableReference( "kur" ).getVariable();
	}
	...
	virtual void fire()
	{
		...
		dy1->setValue( kur->getValue() * (( 1.0 / (20.0 * exp( _v / -11.5 ) + 5.0 * exp( _v / -300.0 )) ) * ( 1.0 - y1->getValue() ) - ( 1.0 / ( 160.0 * exp( _v / 28.0 ) + 200.0 * exp( _v / 1000.0 )) + 1.0 / ( 2500.0 * exp( _v / 20.0 ))) * y1->getValue()) );
		dy2->setValue( kur->getValue() * ((( 1.0 / ( 200.0 * exp( _v / -13.0) + 20.0 * exp( _v / -300.0 ))) * ( 1.0 - y2->getValue() ) - ( 1.0 / ( 1600.0 * exp( _v / 28.0 ) + 2000.0 * exp( _v / 1000.0 )) + 1.0 / ( 10000.0 * exp( _v / 20.0 ))) * y2->getValue() ) ));
		dy3->setValue( kur->getValue() * (( 1.0 / ( 10.0 * exp( _v / 17.0 ) + 2.5 * exp( _v / 300.0 ))) * ( 1.0 - y3->getValue() ) - ( 1.0 / ( 0.35 * exp( _v / -17.0 ) + 2.0 * exp( _v / -150.0 ))) * y3->getValue() ) );
